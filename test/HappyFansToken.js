...
const deployWithoutLGE = deployments.createFixture(async ({ deployments, ethers }, options) => {
  ...
  const pad = await ethers.getContract("HappyFansToken");
...
describe("HappyFansToken", () => {
  describe("Deployment", () => {
    it("name should be HappyFans", async () => {
      const { pad } = await deployWithLGE();
      expect(await pad.name()).to.be.equal("HappyFans");
    });

    it("symbol should be HAPPY", async () => {
      const { pad } = await deployWithLGE();
      expect(await pad.symbol()).to.be.equal("HAPPY");
    });
    ...
    it("total supply should be 100 000 000 000 tokens", async () => {
      const { pad } = await deployWithLGE();
      expect(await pad.totalSupply()).to.be.equal(tokens("100000000000"));
    });

    it("deployer should have the total initial supply", async () => {
      const { pad, deployer } = await deployWithLGE();
      expect(await pad.balanceOf(deployer.address)).to.be.equal(tokens("100000000000"));
    });
  });
